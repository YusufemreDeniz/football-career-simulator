...
public sealed class PromiseAwareDefaultSelectionTests
{
    private static readonly GameDate Day = GameDate.FromCalendarDate(2026, 8, 1);

    [Fact]
    public void DefaultApprove_StartingPromiseOnBench_MovesPlayerIntoXi()
    {
        ...
        var fixtureId = AdvanceToManagedMatchday(modules);
        modules.TeamPrep.ApproveDefaultSelection.Handle(
            new ApproveDefaultMatchSelectionCommand(Guid.NewGuid(), fixtureId, 1));

        var selection = modules.TeamPrep.SelectionStore.Get(new FixtureId(fixtureId), new ClubId(1));
        Assert.NotNull(selection);
        Assert.Contains(benchMember.SlotIndex, selection!.StartingSlotIndices);
    ...
    [Fact]
    public void DefaultApprove_PlayingTimePromiseOutOfMatchday_PutsPlayerOnBench()
    {
        var modules = CreateBound();
        var outMember = modules.TeamPrep.SquadStore.Get(new ClubId(1))!.Members
            .Single(m => m.SlotIndex == MatchSelection.StartingXiSize + MatchSelection.MaxBenchSize);
        ...
        Assert.NotNull(selection);
        Assert.True(
            selection!.StartingSlotIndices.Contains(outMember.SlotIndex)
            || selection.BenchSlotIndices.Contains(outMember.SlotIndex));
    ...
    private static long AdvanceToManagedMatchday(
        (
