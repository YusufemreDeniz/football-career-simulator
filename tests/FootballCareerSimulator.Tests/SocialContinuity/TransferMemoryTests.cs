...
public sealed class TransferMemoryTests : IDisposable
{
    ...
    public TransferMemoryTests() => Directory.CreateDirectory(_tempDirectory);
    ...
    [Fact]
    public void Complete_ClubToClub_CreatesTransferMemoriesForPlayerAndBuyingManager()
    {
        ...
        var transferMemories = social.MemoryStore.Memories
            .Where(m => m.Category == MemoryCategory.Transfer)
            .ToArray();
        Assert.Equal(2, transferMemories.Length);
        Assert.Contains(transferMemories, m => m.RememberingActor.Kind == ActorKind.Player);
        Assert.Contains(transferMemories, m => m.RememberingActor.Kind == ActorKind.Manager);
        Assert.All(transferMemories, m =>
        {
            Assert.Equal(MemorySubjectKind.TransferProcess, m.SubjectKind);
            Assert.Equal(process.ProcessId.Value, m.SubjectId);
            Assert.Equal(MemoryValence.Neutral, m.Valence);
            Assert.Equal(MemoryRecord.TransferCompletedRuleId, m.RuleId);
        });

        transfer.Completion.Complete(process.ProcessId, Day.AddDays(1));
        Assert.Equal(2, social.MemoryStore.Memories.Count(m => m.Category == MemoryCategory.Transfer));
    }

    [Fact]
    public void Complete_FreeAgent_CreatesPositiveTransferMemory()
    {
        ...
        var playerMemory = Assert.Single(
            social.MemoryStore.Memories,
            m => m.Category == MemoryCategory.Transfer && m.RememberingActor.Kind == ActorKind.Player);
        Assert.Equal(MemoryValence.Positive, playerMemory.Valence);
        Assert.True(process.IsFreeAgent);
    }

    [Fact]
    public void SaveLoad_PreservesTransferMemoriesAtSchemaV31()
    {
        var (social, transfer) = CreateThroughFinancialApproval(freeAgent: false);
        var processId = transfer.ProcessStore.Processes.Single().ProcessId;
        transfer.Completion.Complete(processId, Day);

        var path = Path.Combine(_tempDirectory, "transfer-memory.db");
        new CareerSqlitePersistence().Save(
            path,
            WorldCalendarModule.Create(Day, rootSeed: 11).TimelineStore.Timeline,
