...
public sealed class PlayerCareerQueryService
{
    ...
    public ClubDevelopmentSummaryReadModel GetManagedClubSummary()
    {
        ...
        {
            return new ClubDevelopmentSummaryReadModel(null, 0, 0, 0, 0);
        }

        var day = _timelineStore.Timeline.CurrentDate.DayNumber;
        var careers = _store.Careers.Where(c => c.OriginClubId == clubId).ToArray();
        if (careers.Length == 0)
        {
            return new ClubDevelopmentSummaryReadModel(clubId.Value, 0, 0, 0, 0);
        }
        ...
        return new ClubDevelopmentSummaryReadModel(
            ...
            careers.Count(c => c.LastDevelopedOn?.DayNumber == day));
    }
}
